...
function iframeResponse_(e, data) {
  const requestId = String(e && e.parameter ? (e.parameter.rid || '') : '').trim();
  const safeRequestId = requestId.replace(/[^A-Za-z0-9._:-]/g, '').slice(0, 180);

  // Embed JSON safely inside a script block. Replacing the characters below
  // prevents user-controlled text from prematurely closing the script tag.
  const json = JSON.stringify({
    type: 'jota-joti-admin-response',
  ...
  const html = '<!doctype html><html><head><meta charset=\"utf-8\"></head><body>' +
    ...
    'catch(err){try{window.top.postMessage(message, \"*\");}catch(ignore){}}' +
    '})();</script>' +
    '</body></html>';

  return HtmlService.createHtmlOutput(html)
    .setTitle('JOTA-JOTI Admin API')
...
function logBulkEmail_(adminEmail, recipientEmail, user, targetType, subject, status, errorText) {
  try {
    const sheet = getSpreadsheet_().getSheetByName(CONFIG.EMAIL_LOG_SHEET);
    if (!sheet) return;
